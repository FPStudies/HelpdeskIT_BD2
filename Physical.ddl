...
CREATE OR REPLACE FUNCTION find_response_time_f_deprecated (
    p_client_id       IN   NUMBER DEFAULT NULL,
    p_software_name   IN   STRING DEFAULT NULL
) RETURN NUMBER IS
    v_response_time NUMBER := NULL;
BEGIN
    ...
    RETURN v_response_time;
END find_response_time_f_deprecated;
/
...
CREATE OR REPLACE PROCEDURE Delegate_Ticket_p (p_Ticket_Number IN NUMBER, p_Employee_ID IN NUMBER) AS 
BEGIN
...
CREATE MATERIALIZED VIEW active_client_mv
    REFRESH
        FORCE
        START WITH sysdate
        NEXT sysdate + 1 / 2
...
CREATE MATERIALIZED VIEW active_dates_mv
    REFRESH
        FORCE
        START WITH sysdate
        NEXT sysdate + 1 / 2
...
AS
    SELECT
        ...
        to_char(modification_date, 'HH24') AS hour,
        COUNT(*) AS count
    FROM
        archived_tickets
    WHERE
    ...
    GROUP BY
        ROLLUP(to_char(modification_date, 'YYYY'),
               ...
               to_char(modification_date, 'HH24'));
...
CREATE MATERIALIZED VIEW all_messages_count_mv
    REFRESH
        FORCE
        START WITH sysdate
        NEXT sysdate + 1 / 24
...
AS
    SELECT
        to_char(messages.date, 'YYYY') AS year,
        to_char(messages.date, 'MM') AS month,
        to_char(messages.date, 'DD') AS day,
        to_char(messages.date, 'HH24') AS hour,
        software,
        issue_type,
        environment,
        COUNT(*) AS count
    FROM
        messages
        JOIN archived_tickets ON messages.ticket_number = archived_tickets.ticket_number
    GROUP BY
        ROLLUP(messages.ticket_number,
               software,
               issue_type,
               environment,
               to_char(messages.date, 'YYYY'),
               to_char(messages.date, 'MM'),
               to_char(messages.date, 'DD'),
               to_char(messages.date, 'HH24'));
...
CREATE MATERIALIZED VIEW all_new_by_software_mv
    REFRESH
        FORCE
        START WITH sysdate
        NEXT sysdate + 1
...
AS
    SELECT
        software,
        to_char(submission_date, 'YYYY') AS year,
        to_char(submission_date, 'MM') AS month,
        to_char(submission_date, 'DD') AS day,
        to_char(submission_date, 'HH24') AS hour,
        COUNT(*) AS count
    FROM
        tickets
    GROUP BY
        ROLLUP(ticket_number,
               software,
               to_char(submission_date, 'YYYY'),
               to_char(submission_date, 'MM'),
               to_char(submission_date, 'DD'),
               to_char(submission_date, 'HH24'));

CREATE MATERIALIZED VIEW all_new_count_mv
    REFRESH
        FORCE
        START WITH sysdate
        NEXT sysdate + 1 / 24
...
CREATE MATERIALIZED VIEW all_received_count_mv
    REFRESH
        FORCE
        START WITH sysdate
        NEXT sysdate + 1 / 24
...
AS
    ...
    FROM
        filled_tickets
    WHERE
        upper(ticket_status) = 'RECEIVED'
    GROUP BY
        ROLLUP(ticket_number,
               to_char(modification_date, 'YYYY'),
               to_char(modification_date, 'MM'),
               to_char(modification_date, 'DD'),
               to_char(modification_date, 'HH24'));
...
CREATE MATERIALIZED VIEW archived_dates_mv
    REFRESH
        FORCE
        START WITH sysdate
        NEXT sysdate + 1 / 2
...
CREATE MATERIALIZED VIEW client_sla_mv
    REFRESH
        FORCE
        START WITH sysdate
        NEXT sysdate + 1
...
CREATE MATERIALIZED VIEW processing_time_mv
    REFRESH
        FORCE
        START WITH sysdate
        NEXT sysdate + 1
...
AS
    SELECT
        ( modification_date - submission_date ) AS processing_time
    FROM
        archived_tickets
    WHERE
        upper(ticket_status) IN (
...
CREATE MATERIALIZED VIEW raw_dates_mv
    REFRESH
        FORCE
        START WITH sysdate
        NEXT sysdate + 1 / 2
...
CREATE MATERIALIZED VIEW LOG ON raw_tickets NOLOGGING
    STORAGE ( BUFFER_POOL DEFAULT )
NOCACHE
    NOPARALLEL
WITH
...
CREATE OR REPLACE TRIGGER Contracts_Insert_TRG 
...
CREATE OR REPLACE TRIGGER Raw_Tickets_Insert_TRG 
